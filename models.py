...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
  ...
  Args:
    ...
    sample_rate: Number of audio samples per second. # 采样率 16000Hz
    clip_duration_ms: Length of each audio clip to be analyzed.  #音频长度 ms
  ...
  """
  desired_samples = int(sample_rate * clip_duration_ms / 1000) #采样率*音频长度=数据（点数）
  window_size_samples = int(sample_rate * window_size_ms / 1000)
  window_stride_samples = int(sample_rate * window_stride_ms / 1000)
  length_minus_window = (desired_samples - window_size_samples)
  if length_minus_window < 0:  #样本长度小于窗口长度
    spectrogram_length = 0
  else:
    spectrogram_length = 1 + int(length_minus_window / window_stride_samples)   #计算T
